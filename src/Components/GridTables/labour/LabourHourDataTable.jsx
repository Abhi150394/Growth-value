...
const LabourHourChartDataGroupedTable = ({ data, categories=["all"] ,selectedFilterOption = null,searchText = "",  }) => {
  ...
  const { tableData, columnDefs } = useMemo(() => {
    ...
    // extract rows
    const periods = [...new Set(data.detail[categories[0]]?.map((d) => d.hour_of_day))];

    const tableData = periods.map((date) => {
      const row = { date };

      categories.forEach((cat) => {
        const d = data.detail[cat]?.find((x) => x.hour_of_day === date);
        if (!d) return;
