...
const DynamicDropdown = ({
  ...
  icon: TitleIcon,
  options = [],          // [{ value, label: "translation.key" }]
  onChange,
  width = "25%",
  isClearable = false,
  ...
  useEffect(() => {
    const preload = async () => {
      ...
      options.forEach(opt => opt?.label && uniqueLabels.add(opt.label));
      if (title) uniqueLabels.add(title);
      if (placeholder) uniqueLabels.add(placeholder);
      uniqueLabels.add("common.all");

      await Promise.all([...uniqueLabels].map(key => t(key)));
    };

    preload();
  }, [language, options, title, placeholder]);
  ...
  const CustomOption = (props) => (
    <components.Option {...props}>
      <Box display="flex" alignItems="center" gap={1}>
        {props.data.icon && (
          <props.data.icon style={{ fontSize: SZ.icon }} />
        )}
        <TranslatedText>{props.data.label}</TranslatedText>
      </Box>
    </components.Option>
  );
  ...
  const CustomSingleValue = (props) => (
    <components.SingleValue {...props}>
      <Box display="flex" alignItems="center" gap={SZ.singleValueGap / 8}>
        {TitleIcon && (
          <TitleIcon style={{ fontSize: SZ.titleIcon }} />
        )}
        {title && (
          <span style={{ fontWeight: FONT_WEIGHT?.lg || 600, marginRight: 4 }}>
  ...
  const CustomMultiValueLabel = (props) => (
    <components.MultiValueLabel {...props}>
      <span style={{ fontSize: SZ.chipFont }}>
        <TranslatedText>{props.data.label}</TranslatedText>
      </span>
  ...
  const computedPlaceholder =
    !isMulti && hasTitleAndIcon
      ? tSync("common.all")
      : tSync(placeholder || options[0]?.label || "");
  ...
  return (
    <Box width={{ xs: "100%", sm: width }} minWidth={140}>
      <Select
        isMulti={isMulti}
        options={options}
        value={selected}
        ...
        components={{
          Option: CustomOption,
          ...(hasTitleAndIcon && !isMulti && { SingleValue: CustomSingleValue }),
          ...(isMulti && {
